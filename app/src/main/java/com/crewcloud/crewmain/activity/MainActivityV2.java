...
public class MainActivityV2 extends BaseActivity implements NavigationView.OnNavigationItemSelectedListener, DevicePresenter.view {
    ...
    @BindView(R.id.textViewApproval)
    TextView tvApproval;
    ...
    @BindView(R.id.fl_enabled_applications)
    RecyclerView rvApplication;

    @BindView(R.id.icDropApproval)
    ImageView icDropApproval;

    @BindView(R.id.icDropMail)
    ImageView icDropMail;

    @BindView(R.id.icDropSchedule)
    ImageView icDropSchedule;

    @BindView(R.id.icDropNotice)
    ImageView icDropNotice;

    @BindView(R.id.icDropCommunity)
    ImageView icDropCommunity;

    @BindView(R.id.icDropApplication)
    ImageView icDropApplication;
...
//                URL apkUrl = new URL("http://www.crewcloud.net/Android/Package/CrewMain.apk");
    ...
    private void initView() {
        ...
        icDropApproval.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rv_approval.getVisibility() == View.VISIBLE) {
                    rv_approval.setVisibility(View.GONE);
                    icDropApproval.setRotation(270L);
                } else {
                    rv_approval.setVisibility(View.VISIBLE);
                    icDropApproval.setRotation(0L);
                }
            }
        });

        icDropMail.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rv_unread_mail.getVisibility() == View.VISIBLE) {
                    rv_unread_mail.setVisibility(View.GONE);
                    icDropMail.setRotation(270L);
                } else {
                    rv_unread_mail.setVisibility(View.VISIBLE);
                    icDropMail.setRotation(0L);
                }
            }
        });

        icDropSchedule.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rv_schedule.getVisibility() == View.VISIBLE) {
                    rv_schedule.setVisibility(View.GONE);
                    icDropSchedule.setRotation(270L);
                } else {
                    rv_schedule.setVisibility(View.VISIBLE);
                    icDropSchedule.setRotation(0L);
                }
            }
        });

        icDropNotice.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rv_notice.getVisibility() == View.VISIBLE) {
                    rv_notice.setVisibility(View.GONE);
                    icDropNotice.setRotation(270L);
                } else {
                    rv_notice.setVisibility(View.VISIBLE);
                    icDropNotice.setRotation(0L);
                }
            }
        });

        icDropCommunity.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rvCommunity.getVisibility() == View.VISIBLE) {
                    rvCommunity.setVisibility(View.GONE);
                    icDropCommunity.setRotation(-90L);
                } else {
                    rvCommunity.setVisibility(View.VISIBLE);
                    icDropCommunity.setRotation(0L);
                }
            }
        });

        icDropApplication.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                if(rvApplication.getVisibility() == View.VISIBLE) {
                    rvApplication.setVisibility(View.GONE);
                    icDropApplication.setRotation(-90L);
                } else {
                    rvApplication.setVisibility(View.VISIBLE);
                    icDropApplication.setRotation(0L);
                }
            }
        });
    ...
    private class WebClientAsync_GetEnabledApplications extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            try {
                ...
                if (CrewCloudApplication.getInstance().getPreferenceUtilities().getStringValue(Constants.COMPANY_NAME, "").equals(Constants.CHECK_DOMAIN_HANMARU)) {
                    Application eis = new Application();
                    eis.setProjectCode(Constants.PROJECT_CODE_HANMARU);
                    eis.setPackageName(Constants.URL_HANMARU);
                    eis.setApplicationName(getResources().getString(R.string.hanmaru));
                    lstApp.add(eis);
                }
        ...
        if (intent == null) {
            if (application.getProjectCode().equals("OA") || application.getProjectCode().equals("HappyCall")) {
                ...
                if(application.ProjectCode.equals(Constants.PROJECT_CODE_COFFEE)
                        || application.ProjectCode.equals(Constants.PROJECT_CODE_PMS)
                        || application.ProjectCode.equals(Constants.PROJECT_CODE_HANMARU)) {
